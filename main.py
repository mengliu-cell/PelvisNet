...
#  get_model 函数根据配置 (cfg) 动态创建模型对象。如果配置了额外的编码器 (extra_encoder)，则加载相应的模块并与主模型结合
def get_model(cfg):
...
# 返回一个 PyTorch Lightning 模块，用于封装模型、损失函数、优化器、指标等
...
def main(cfg):
    # 首先初始化日志记录器，然后加载数据、创建模型、定义评价指标，最后使用 PyTorch Lightning 的 Trainer 类进行训练
